...
### 文字の大きさを変える
`<style>` の `:root` にある `--base-size` が全体の基準です。ここを変えると、見出しから注釈まで比率を保ったまま拡大・縮小します。

```css
--base-size: 50px;   /* 大きくするなら 54px、控えめにするなら 46px */
```

会場が広い・後方席が遠いときは `54px` 程度まで上げられます。ただし付録①のプロンプト集や付録②のフローチャートは行数が多いので、上げすぎると枠からはみ出します。上げたら必ずその2枚を確認してください。
